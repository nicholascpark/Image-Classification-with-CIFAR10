...
def main():
    ...
    # per_cls_weights = torch.from_numpy(np.ones(10)).cuda()

    if args.imbalance == "regular":
        train_dataset = torchvision.datasets.CIFAR10(
            root='./data', train=True, download=True, transform=transform_train)
        # if not args.reweight:
        #     per_cls_weights = torch.from_numpy(np.ones(10))
        #     if torch.cuda.is_available():
        ...
        print("class rate:", np.array(cls_num_list)/sum(cls_num_list))
        if args.reweight:
            per_cls_weights = reweight(cls_num_list, beta=args.beta)
            if torch.cuda.is_available():
                per_cls_weights = per_cls_weights.cuda()
        else:
            per_cls_weights = reweight(cls_num_list, beta=0)
            if torch.cuda.is_available():
                per_cls_weights = per_cls_weights.cuda()
